...
class  expand_modelModel extends commonModel
{
    ...
    public function field_add($data) {
        ...
        //添加真实字段
        $sql="
        ALTER TABLE {$this->model->pre}expand_content_{$model['table']} ADD {$data['field']} {$property['name']}({$data['len']}{$decimal}) DEFAULT NULL
        ";
        $this->model->query($sql);
    ...
    public function field_edit($data) {
        ...
        //添加真实字段
        $sql="
        ALTER TABLE {$this->model->pre}expand_content_{$model['table']} CHANGE {$info['field']} {$data['field']} {$property['name']}({$data['len']}{$decimal})
        ";
        $this->model->query($sql);
        $data['default']=html_in($data['default']);
        $data['config']=html_in($data['config']);
    ...
    public function get_field_html($info,$data=null){
        ...
        switch ($info['type']) {
            ...
            case '3':
                ...
                <tr>
                    <td align="right">'.$info['name'].'</td>
                    <td colspan="2"><textarea name="'.$info['field'].'" style="width:100%; height:350px;" id="'.$info['field'].'">'.$info['default'].'</textarea></td>
                </tr>
                ';
