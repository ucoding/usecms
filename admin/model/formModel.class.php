...
class formModel extends commonModel
{
    ...
    public function form_list() {
        return $this->model->table('form')->order('id asc')->select();
    }
    ...
    public function field_add($data)
    {
        ...
        //添加真实字段
        $sql="
        ALTER TABLE {$this->model->pre}form_data_{$model['table']} ADD {$data['field']} {$property['name']}({$data['len']}{$decimal}) DEFAULT NULL
        ";
        $this->model->query($sql);
    ...
    public function field_edit($data)
    {
        ...
        //修改真实字段
        $sql="
        ALTER TABLE {$this->model->pre}form_data_{$model['table']} CHANGE {$info['field']} {$data['field']} {$property['name']}({$data['len']}{$decimal})
        ";
        $this->model->query($sql);
